...
async function getListUser() {
  ...
  if (res) {
    const filterUser = filter(res.data.data, (user) => user.role !== "ADMIN");
    dataSource.data = sortBy(filterUser, "id");
    originUserList.value = dataSource.data;
  }
}
...
watch(isActive, () => {
  if (!isActive.value) {
    ...
    dataSource.data = originUserList.value;
  }
});
</script>
    ...
    <div class="user__filter">
      <div class="flex">
        ...
        <Switch v-model:checked="isActive">
          ...
          <template #unCheckedChildren>Deactive</template>
        </Switch>
      </div>
      ...
      >
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-if="column.dataIndex === 'role'">
            ...
            <div v-if="record.role === 'STAFF'" class="role role__u">
              {{ convertRole(record.role) }}
            </div>
          </template>
          ...
          <Form :model="reqParams" name="normal_login" class="login-form">
            <div class="flex justify-between">
              ...
              <div class="w-[48%]">
                ...
                <div>
                  <label class="mb-2">
                    Salary <span class="text-red-600">&ast;</span>
                    <!-- <span class="note--salary">(VND)</span> -->
                  </label>
                  <FormItem>
                    <InputNumber
                      class="w-full"
