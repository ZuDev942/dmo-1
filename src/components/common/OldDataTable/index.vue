        ...
        <table class="table-auto dataTable__table">
          ...
          <tbody v-if="dataSource.data && dataSource.data.length !== 0">
            <tr v-for="(row, rowIndex) in dataSource.data" :key="rowIndex">
              <td
                v-for="(column, columnIndex) in dataSource.columns"
                :key="columnIndex"
              >
                <slot
                ...
                ></slot>
                <span v-else>{{
                  getColumnValue(row, rowIndex, column, columnIndex)
                }}</span>
              </td>
            </tr>
          </tbody>
          ...
          <tbody v-else>
            <tr>
              <td
                class="dataTable__noData"
                :colspan="dataSource.columns.length"
              >
      ...
      <div class="dataTable__footer">
        <div v-if="paginator.total > 0" class="paginator">
          <div class="paginator__info">
            <b
              >{{ indexFromToPaginator.from }}-{{ indexFromToPaginator.to }} /
              {{ paginator.total }}
            </b>
            {{ paginator.textHint }}
          </div>
    ...
    getColumnValue(
      ...
      if (this.dataSource.columns[columnIndex]) {
        const columnKey: string | undefined =
          this.dataSource.columns[columnIndex].key;
